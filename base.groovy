...
while(projects.hasNext()) {
	...
	* "compilers":"gcc,vs2013", */
	 ...
	 */
	String email = json.email ?: "scarlett.gately.clark@gmail.com"
	...
	*/
	...
	branchgroups.each { branchGroup ->
		...
		customWorkspace('${JENKINS_BUILDS}' + '/' + "${jobname}" + '/' + "${branchGroup}")
		configure { project ->
			project << childCustomWorkspace(".")
		}		
		
		// Set only build on slaves assignment to be sure we are on the correct OS.
		// Set qt and qt5 builds on LINBUILDER2 aka qtbuilder(more power)
		if (jobname == 'qt5' || jobname == 'qt4' || jobname == 'calligra' || jobname == 'kdepim') {
			configure { project ->
				project.name = 'matrix-project'
				project / 'properties' << 'jp.ikedam.jenkins.plugins.groovy_label_assignment.GroovyLabelAssignmentProperty' {
					groovyScript 'def labelMap = [ Linux: "QTBUILDER", Windows: "WINBUILDER", OSX: "OSXQTBUILDER"]; return labelMap.get(binding.getVariables().get("PLATFORM"));'
				}
			}
		} else {
			configure { project ->
