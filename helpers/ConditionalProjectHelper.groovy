...
class ConditionalProjectHelper {
						...
						return { project ->
							project / buildWrappers << 'org.jenkinsci.plugins.preSCMbuildstep.PreSCMBuildStepsWrapper' {
									...
									'org.jenkinsci.plugins.conditionalbuildstep.singlestep.SingleConditionalBuilder' {
										...
										runner(class: "org.jenkins_ci.plugins.run_condition.BuildStepRunner\$Fail")
										buildStep(class: 'hudson.tasks.Shell') {
											command 'git clone https://gerrit-ring.savoirfairelinux.com/ring-daemon \n' \
											+ 'cp -a ring-daemon/src/dring /tmp/'
										}
									}
								}
							}
	...
	static String commandBuilder(String jobname, String platform, ArrayList compiler, String projectname, String branchGroup) {
		...
		def lindefaultbasebuildcommand = 'python -u ${JENKINS_SLAVE_HOME}/tools/perform-build.py ' + '--project ' + "${projectname}" + ' --branchGroup ' + "${branchGroup}"	+ ' --compiler ' + 'gcc' + ' --platform ' + 'linux64-g++' + ' --sources ' + '${WORKSPACE}'									
		def osxdefaultbasebuildcommand = 'python2.7 -u ${JENKINS_SLAVE_HOME}/tools/perform-build.py ' + '--project ' + "${projectname}" + ' --branchGroup ' + "${branchGroup}" + ' --compiler ' + 'clang' + ' --platform ' + 'darwin-mavericks' + ' --sources ' + '${WORKSPACE}'	
		...
		if (platform == "Windows" && compiler.contains('vs2013')){			
			...
			} else if (jobname == 'ring') {
				jobcustomcommand = 'cd contrib \n' + 'mkdir native \n' + 'cd native \n' + '../bootstrap \n' + 'make -j \n' \
				+ 'cd ../.. \n' + './autogen.sh \n'
			}
			...
			} else if (jobname == "qca" || jobname == "ark") {
				jobcustomcommand = 'export CFLAGS="-isystem/opt/local/include" \n' + 'export CXXFLAGS="-isystem/opt/local/include" \n'
